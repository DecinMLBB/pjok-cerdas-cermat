...
const state = {
  screen: "home",
  user: readJSON("pjokArenaUserV2", null),
...
function avatarStyle(index = 1, size = 80) {
  ...
  return `width:${size}px;height:${size}px;background-image:url("${ASSET.avatars}");background-size:${size * 10}px ${size * 10}px;background-position:${-col * size}px ${-row * size}px;background-repeat:no-repeat;`;
}
function avatarDiv(index = 1, cls = "avatar") {
  const size = cls.includes("mini") ? 34 : cls.includes("profile") ? 96 : 76;
  return `<div class="avatar-sprite ${cls}" style="${avatarStyle(index, size)}"></div>`;
}
...
function profile() {
  ...
  shell(`<section>
    ...
    ${pageHead("PROFILE", "Pilih identitas pemain arena")}
    <div class="panel profile-panel">
      ...
      <div class="avatar-grid">${Array.from({length:100}, (_, n) => {
        const i=n+1;
        return `<button class="avatar-choice ${i===selected?"selected":""}" data-avatar-index="${i}" aria-label="Avatar ${i}"><span class="avatar-cell" style="${avatarStyle(i,62)}"></span><small>${String(i).padStart(2,"0")}</small></button>`;
      }).join("")}</div>
      <div class="form-grid">
    ...
    <div class="hint-card">Profil disimpan di perangkat ini. Jika Supabase sudah diaktifkan, skor juga dapat masuk ke leaderboard online.</div>
  </section>`, "profile");
}
...
async function saveProfile() {
  ...
  state.user = { username, fullName, className, avatarIndex: Number(state.pendingAvatarIndex || 1) };
  saveJSON("pjokArenaUserV2", state.user);
...
function leaderboard() {
  ...
  const rows = mergeRows(online.length ? online : local);
  shell(`<section>
...
async function refreshOnlineLeaderboard() {
  ...
  try {
    ...
    if (box && state.screen === "leaderboard") box.innerHTML = leaderboardContent(cleanRows(state.onlineRows));
  } catch (e) {
...
function startQuiz() {
  ...
  let pool = state.selectedCategory === "Semua" ? all : all.filter(q => q.c === state.selectedCategory);
  if (!pool.length) pool = all;
  pool = shuffle(pool).slice(0, Math.min(10, pool.length));
  state.quiz = { items:pool, index:0, score:0, correct:0, combo:0, bestCombo:0, time:15, timer:null, locked:false };
  renderQuiz();
}
function renderQuiz() {
...
async function ensureAuth() {
  if (!sb || state.authReady) return;
  try {
    const {data:{session}} = await sb.auth.getSession();
    if (!session) await sb.auth.signInAnonymously();
    state.authReady=true;
  } catch (e) { console.warn("Supabase auth unavailable:", e.message || e); }
}
...
document.addEventListener("click", e => {
  ...
  if(av){
    ...
    const p=$(".profile-picker-preview"); if(p)p.setAttribute("style",avatarStyle(state.pendingAvatarIndex,96));
    beep(720,.05); return;
  }
  ...
  else if(a==="clearLocal"){ if(confirm("Hapus profil dan riwayat skor di perangkat ini?")){ localStorage.removeItem("pjokArenaUserV2");localStorage.removeItem("pjokArenaResults");state.user=null;state.results=[];toast("Data lokal dihapus.","ok");home();} }
});
