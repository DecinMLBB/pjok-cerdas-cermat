...
/* ===== PJOK ARENA BACKSOUND — YOUTUBE IFRAME API ===== */
const PJOK_MUSIC_VIDEO = "BpcdiYYEmvE";
let pjokMusicOn = localStorage.getItem("pjokMusicOn") !== "0";
let pjokYTPlayer = null;
let pjokYTReady = false;
let pjokUserInteracted = false;

function pjokMusicMount(){
  if(document.getElementById("pjok-yt-music")) return;
  const wrap=document.createElement("div");
  wrap.id="pjok-music-wrap";
  wrap.innerHTML=`<div id="pjok-yt-music" aria-hidden="true"></div>`;
  document.body.appendChild(wrap);
}

function pjokLoadYouTubeAPI(){
...
window.onYouTubeIframeAPIReady=function(){
  pjokYTPlayer=new YT.Player("pjok-yt-music",{
    width:200,height:200,
    videoId:PJOK_MUSIC_VIDEO,
    playerVars:{
      autoplay:0,
      controls:0,
      loop:1,
      playlist:PJOK_MUSIC_VIDEO,
      playsinline:1,
      rel:0,
    ...
    events:{
      onReady:function(e){
        ...
        if(pjokMusicOn && pjokUserInteracted) pjokMusicPlay();
      },
      onStateChange:function(e){
        if(e.data===0 && pjokMusicOn) e.target.playVideo();
      },
      onError:function(e){
        console.warn("PJOK music YouTube error:",e.data);
      },
      onAutoplayBlocked:function(){
        // Browser requires a real user gesture; the global interaction listener retries it.
      }
    }
  });
};

function pjokMusicPlay(){
  if(!pjokYTPlayer || !pjokYTReady || !pjokMusicOn) return;
  try{
    pjokYTPlayer.unMute();
    pjokYTPlayer.setVolume(65);
    pjokYTPlayer.playVideo();
  }catch(err){ console.warn("PJOK music play failed",err); }
}

function pjokMusicPause(){
  if(!pjokYTPlayer || !pjokYTReady) return;
  try{ pjokYTPlayer.pauseVideo(); }catch(err){}
}

function pjokMusicRefreshButton(){
  const b=document.getElementById("pjok-music-btn");
  if(!b) return;
  b.textContent=pjokMusicOn ? "🔊" : "🔇";
  b.title=pjokMusicOn ? "Matikan backsound" : "Nyalakan backsound";
  b.setAttribute("aria-label",b.title);
}

function pjokMusicToggle(){
  pjokMusicOn=!pjokMusicOn;
  localStorage.setItem("pjokMusicOn",pjokMusicOn?"1":"0");
  pjokMusicRefreshButton();
  if(pjokMusicOn){
    pjokUserInteracted=true;
    pjokMusicPlay();
  }else{
    pjokMusicPause();
  }
...
function pjokMusicInit(){
  ...
  // First tap/click anywhere counts as the user gesture needed by mobile browsers.
  const firstInteraction=()=>{
    pjokUserInteracted=true;
    if(pjokMusicOn) pjokMusicPlay();
  ...
  document.addEventListener("keydown",firstInteraction,{passive:true});
}

// Let the existing top-right sound button control the same music switch.
const _oldSoundHandlerNote = true;
const _pjokOriginalClickHandler = document.onclick;
document.addEventListener("click",function(e){
  const topSound=e.target.closest?.('#soundBtn');
  if(topSound){
    e.preventDefault();
    e.stopPropagation();
    pjokMusicToggle();
